...
const Header = () => {
  ...
  const getCategoriesData = async (page) => {
    try {
      ...
      if (data && Array.isArray(data.data)) {
        ...
        // Translate the newly fetched data if CurrentLanguage is available
        if (!isEmptyObject(CurrentLanguage)) {
          newData = translateCategories(newData);
        }
        ...
        if (page > 1) {
          ...
          const uniqueNewData = newData.filter(cat => !existingIds.has(cat.id));
          
          // Only append unique new data items
          dispatch(setCateData([...cateData, ...uniqueNewData]));
        } else {
          dispatch(setCateData(newData));
  ...
  useEffect(() => {
    if (cateData.length > 0 && !isEmptyObject(CurrentLanguage)) {
      // Only translate when language changes, not when cateData length changes
      // This prevents duplicates when scrolling loads more items
