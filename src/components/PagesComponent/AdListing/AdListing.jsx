...
const AdListing = () => {
  ...
  const loadProfileLocation = () => {
    ...
    if (storedLocation) {
      setCountryStore(prev => ({ ...prev, SelectedCountry: storedLocation.country }));
      setStateStore(prev => ({ ...prev, SelectedState: storedLocation.state }));
      setCityStore(prev => ({ ...prev, SelectedCity: storedLocation.city }));
      setAddress(storedLocation.address);
      setLocation({
        country: storedLocation.country.name,
        state: storedLocation.state.name,
        city: storedLocation.city.name,
  ...
  useEffect(() => {
    // Load profile location on mount if available
    loadProfileLocation();
  }, []);
  ...
  const handleAdListingChange = (e) => {
    ...
    setAdListingDetails((prevDetails) => {
      const updatedDetails = {
        ...prevDetails,
        [name]: name === 'price' || name === 'discounted_price' ? (value === '' ? '' : Number(value)) : value,
      };
      if (name === 'title') {
        updatedDetails.slug = generateSlug(value);
      }
      // Reset discounted price if it's higher than the new price
      if (name === 'price' && updatedDetails.discounted_price > Number(value)) {
        updatedDetails.discounted_price = '';
      }
      // Clear discounted price if price is cleared
      if (name === 'price' && value === '') {
        updatedDetails.discounted_price = '';
      }
  ...
  const handleDetailsSubmit = () => {
    ...
    else if (AdListingDetails.discounted_price && (AdListingDetails.discounted_price < 0 || AdListingDetails.discounted_price > AdListingDetails.price)) {
      toast.error(t('enterValidDiscountPrice'));
      return;
    }
  ...
  const postAd = async () => {
    ...
    const allData = {
      ...
      discounted_price: AdListingDetails.discounted_price || null, // Send null if empty
    ...
    try {
      setIsAdPlaced(true)
      console.log('Sending data:', allData); // Add this to debug
  ...
  const handleFullSubmission = () => {
    ...
    const { title, desc, price, phonenumber, slug, link, discounted_price } = AdListingDetails;
    ...
    if (discounted_price && (discounted_price < 0 || discounted_price > price)) {
      toast.error(t('enterValidDiscountPrice'));
      setActiveTab(2);
      return;
    }
...
//   const getAreaData = async (search, page) => {
//     const params = { city_id: CityStore?.SelectedCity?.id };
//     if (search) params.search = search;
//     else params.page = page;
//     try {
//       const res = await getAreasApi.getAreas(params);
//       let allArea = page > 1 ? [...AreaStore?.Areas, ...res?.data?.data?.data] : res?.data?.data?.data;
//       setAreaStore(prev => ({
//         ...prev,
//         currentPage: res?.data?.data?.current_page,
//         Areas: allArea,
...
//         if (!address) address = result.formatted_address;
//       });
//       const locationData = { lat: pos.lat, long: pos.lng, city, state, country, address };
//       setLocation(locationData);
//     } catch (error) {
//       console.error('Error fetching location data:', error);
//     }
...
//               if (!address) address = result.formatted_address;
//             });
//             const cityData = { lat: locationData.latitude, long: locationData.longitude, city, state, country, address };
//             setLocation(cityData);
//           } catch (error) {
//             console.error('Error fetching location data:', error);
//           }
...
//   useEffect(() => {
//     if (activeTab === 2 && allCategoryIdsString) {
//       getCustomFieldsData();
//     }
//   }, [allCategoryIdsString]);
...
//   const handleGoBack = () => {
//     setActiveTab((prev) => {
//       if (CustomFields.length === 0 && activeTab === 4) return prev - 2;
//       return prev - 1;
//     });
//   };
...
//     const { name, value } = e.target;
//     setAdListingDetails((prevDetails) => {
//       const updatedDetails = { ...prevDetails, [name]: name === 'price' || name === 'discounted_price' ? (value === '' ? '' : Number(value)) : value };
//       if (name === 'title') updatedDetails.slug = generateSlug(value);
//       if (name === 'price' && updatedDetails.discounted_price > Number(value)) updatedDetails.discounted_price = '';
//       if (name === 'price' && value === '') updatedDetails.discounted_price = '';
//       return updatedDetails;
//     });
//   };
...
//       return;
//     }
//     if (CustomFields?.length === 0) setActiveTab(4);
//     else setActiveTab(3);
//   };

//   const submitExtraDetails = (e) => {
//     if (!validateExtraDetails(CustomFields, extraDetails)) return;
//     setActiveTab(4);
//   };
...
//       all_category_ids: allCategoryIdsString,
//       price: AdListingDetails.price,
//       discounted_price: AdListingDetails.discounted_price || null,
//       contact: AdListingDetails.phonenumber,
//       video_link: AdListingDetails?.link,
//       custom_fields: transformedCustomFields,
//       image: uploadedImages[0],
...
//   const handleTabClick = (tab) => {
//     if (tab === 1 && !DisabledTab.selectCategory) setActiveTab(1);
//     else if (tab === 2 && !DisabledTab.details) setActiveTab(2);
//     else if (tab === 3 && !DisabledTab.extraDet) setActiveTab(3);
//     else if (tab === 4 && !DisabledTab.img) setActiveTab(4);
//     else if (tab === 5 && !DisabledTab.loc) setActiveTab(5);
//   };
...
//             </div>
//           </div>
//           {(activeTab === 1 || activeTab === 2) && CurrentPath.length > 0 && (
//             <div className="col-12">
//               <div className="tabBreadcrumb">
//                 <span className='title1'>{t('selected')}</span>
//                 <div className='selected_wrapper'>
//                   {CurrentPath.map((item, index) => (
//                     <span className='title2' key={item.id} onClick={() => handleSelectedTabClick(item?.id)}>
//                       {item.name}{index !== CurrentPath.length - 1 && CurrentPath.length > 1 ? ',' : ''}
//                     </span>
//                   ))}
//                 </div>
//               </div>
...
//                     loadProfileLocation={loadProfileLocation} // Pass the function as a prop
//                   />
//                 )}
//               </div>
//             </div>
