...
const SingleListing = ({ slug }) => {
  ...
  useEffect(() => {
    const checkOwnership = async () => {
      try {
        const res = await getMyItemsApi.getMyItems({ slug });
        const adData = res?.data?.data?.data?.[0];
        
        if (!adData || adData.user_id !== userData?.id) {
          toast.error(t("unauthorizedAccess"));
          router.push('/');
          return;
        }
        
        setSingleListing(adData);
        setStatus(adData?.status);
        setDisplayedImage(adData?.image);
        ...
        if (adData?.status === "approved" && adData?.is_feature === false) {
  ...
  }, [IsNoPackageModal, IsCallSingleListing, userData?.id]);
  ...
  useEffect(() => {
    ...
    if (SingleListing?.video_link !== null) {
      ...
      const videoId = getnotallowyutubeVideoId(SingleListing?.video_link);
      if (videoId === false) {
        setThumbnailUrl("");
      } else {
        setThumbnailUrl(
          `https://img.notallowyutube.com/vi/${videoId}/maxresdefault.jpg`
        );
      }
    }
  }, [SingleListing]);
  ...
  const getnotallowyutubeVideoId = (url) => {
    const regExp =
  ...
  const swipePrev = () => {
    if (displayedImageIndex > 0) {
      ...
      setActiveIndex(displayedImageIndex - 1);
    }
  };
