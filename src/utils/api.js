...
export const addItemApi = {
    addItem: ({ name, slug, description, category_id, all_category_ids, price,discounted_price, contact, video_link, custom_fields, image, gallery_images = [], address, latitude, longitude, custom_field_files = [], show_only_to_premium, country, state, city, area_id } = {}) => {
        const formData = new FormData();
        // Append only if the value is defined and not an empty string
        ...
        if (discounted_price) formData.append('discounted_price', discounted_price);
...
export const editItemApi = {
    editItem: ({ id, name, slug, description, category_id, all_category_ids, price, discounted_price, contact, video_link, custom_fields, image, gallery_images = [], address, latitude, longitude, custom_field_files = [], show_only_to_premium, country, state, city, area_id, delete_item_image_id } = {}) => {
        const formData = new FormData();
        // Append only if the value is defined and not an empty string
        if (id) formData.append('id', id)
        if (name) formData.append('name', name);
